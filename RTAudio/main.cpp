...
{
    unsigned int channels = 1, fs = 8, bufferFrames, device = 1, offset = 0;
    double time = 2.0;
    FILE *fd;
   ...
   /* if ( argc < 3 || argc > 6 ) 
    ...
    int devicecount = adc.getDeviceCount();
     cout << "adc.getDeviceCount(): " << adc.getDeviceCount() << endl;
    if ( devicecount < 1 ) {
        cout << "adc.getDeviceCount(): " << adc.getDeviceCount() << "\tNo audio devices found!\n";
        exit( 1 );
    }
...
*/
    ...
    cout << "\nRecording for " << time << " seconds ... writing file 'record.wav' (buffer frames = " << bufferFrames << ")." << endl;
